...
| `IMAGE_LLAMACPP` | `ghcr.io/ggml-org/llama.cpp:server` | llama.cpp image |
| `IMAGE_LLAMACPP_GPU` | `ghcr.io/ggml-org/llama.cpp:server-cuda` | llama.cpp CUDA image |
| `IMAGE_VLLM` | `docker.io/vllm/vllm-openai:latest` | vLLM image |
| `IMAGE_OLLAMA` | `docker.io/ollama/ollama:latest` | Ollama image |

A JSON API mirrors everything the UI does under `/api/v1/` (deployments,
models, downloads, stats, health).
